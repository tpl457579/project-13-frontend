...
const GuessTheCat = () => {
  ...
  const preloadImages = useCallback(async (count = 3) => {
    const fetches = Array.from({ length: count * 2 }, () =>
      fetch('https://cat.ceo/api/breeds/image/random').then((res) => res.json())
    )
  ...
  useEffect(() => {
    if (started && !gameOver) fetchCats()
  }, [started, gameOver, fetchCats])
  ...
  const handleGuess = useCallback(
    (url) => {
      ...
      setTimeout(fetchCats, 500)
    },
    [correctBreed, gameOver, clickLocked, paused, fetchCats]
  )

  useEffect(() => {
    if (!started || paused || gameOver) return
    if (imagesLoaded < catImages.length || catImages.length === 0) return
    if (timerRef.current) clearInterval(timerRef.current)
    timerRef.current = setInterval(() => dispatch({ type: 'TICK' }), 1000)
    return () => clearInterval(timerRef.current)
  }, [started, paused, gameOver, imagesLoaded, catImages.length])
  ...
  }, [timer, started, paused, gameOver, fetchCats])
  ...
  }, [catImages, loading, firstLoad, handleGuess])
